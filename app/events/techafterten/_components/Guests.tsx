...
const GuestCard = (data: Omit<TGuestProps, "notPublicYet"> & { onClick: () => void }) => {
    return <div
...
const NotPublicYetGuestCard = (props: Pick<TGuestProps, "placeholder"> & { className?: string }) => {
    return (
        <div className={
            cn("privateGuestCard bg-gradient-to-br from-gray-200 to-blue-blue/50 h-full w-full flex justify-center items-center animate-pulse",
                props.className
            )}>
            {props.placeholder}
        </div>
...
type TGuestModalParam = TGuestProps & { modalState: boolean, closeModal: () => void } & Pick<GuestSectionProps, "className">;
        ...
        <div className={"text-center md:text-left p-small  py-standard w-full max-w-[1400px] md:px-standard brk-1400:mx-auto sm:px-block"}>
            ...
            <div className="speakerDetails flex justify-between flex-wrap-reverse gap-small mt-5">
                ...
                <div className="imageSocials w-auto flex flex-col justify-center items-center md:items-start mx-auto md:mx-0">
                    <img src={props.image} alt={props.name} className="image max-w-52 aspect-square object-cover rounded-md" />
...
const GuestsSection = ({ guestData, sectionTitle, className }: GuestSectionProps) => {
    ...
    return (
        <section className={cn("speakers_section ",
                ...
                {!!guestData?.length && guestData?.map((guest) =>
                    guest.notPublicYet
                        ...
                        : <GuestCard
                            key={guest.id}
                            {...guest}
                            onClick={() => setGuestModal({ visible: true, active: guest.id })} />
                )}
            </div>
            ...
            <div className={`ModalContainer w-full min-h-96 bg-white/80 backdrop-blur-md fixed bottom-0 left-0  z-50 [box-shadow:0px_-20px_50px_rgba(0,0,0,0.18)]
                            ...
                            ${guestModal.visible ? "translate-y-0 opacity-100" : "translate-y-full opacity-40"} `}
            >
                {guestData?.filter((guest) => guest.id === guestModal.active).map((speaker) => (
                    <GuestModal
