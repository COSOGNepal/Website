...
export function Prize(props: TPrizeProps & { className?: Record<string, string>, isTopPrize: boolean, index: number }) {
    return (
        <div className={cn(
        ...
        >
            <Icon iconName={props.prizeIcon} className={cn("h-44 saturate-50 opacity-20 absolute top-standard left-0 ", props.className?.icon)} />
            <div className={cn("details", props.className?.detailsContainer)}>
                {props.prizeDescription}
                <div className={cn("title text-empactathon-dark text-xl font-bold",
